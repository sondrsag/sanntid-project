...
int de_serialize_job_from_buffer(char* buffer, Job_t *job) {
    int chksum;
    int result =
        sscanf(buffer, "job %d %d %d %d %d", &chksum, &(job->floor), (int*)&(job->button),
               (int*)&(job->finished), &(job->assignee));
...
int de_serialize_OutsideCallsList_from_buffer(char const * buffer, OutsideCallsList_t calls_list) {
     ...
     */
    return 0;
} /* de_serialize_OutsideCallsList_from_buffer */
