...
int   ReturnElevatorId(ElevatorStatus_t *     All_elevators,
                       elev_motor_direction_t call_direction,
                       int                    call_floor);
                     ...
                     int                    call_floor)
{
    ...
    for (int i_e = 0; i_e < NUM_ELEVATORS; i_e++) { //If no idle, find elevator in same direction which has not reached this floor yet
        if (All_elevators[i_e].available == true && All_elevators[i_e].direction ==
            call_direction &&
