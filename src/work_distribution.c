...
static int FindIdleElevator(ElevatorStatus_t *All_elevators)
{
    for (int i_e = 0; i_e < NUM_ELEVATORS; ++i_e) {
        if (All_elevators[i_e].available &&
            All_elevators[i_e].action == IDLE) {
...
void Handle_jobs_assigned()
{
    for (int i = 0; i < NUM_FLOORS; i++) {
        ...
        if (OutsideCallsList[i].up == true &&
            OutsideCallsList[i].el_id_up == local_assignee_id) {
            dummy_job.floor    = i;
        ...
        pthread_mutex_lock(&wd_mtx);
        if (OutsideCallsList[i].down == true &&
            OutsideCallsList[i].el_id_down == local_assignee_id) {
            dummy_job.floor    = i;
...
} /* wd_HandleInternalCallsAfterRestart */
...
void wd_receiveJob_from_local_elevator(Job_t job)
{
    ...
    pthread_mutex_unlock(&wd_mtx);
} /* wd_receiveJob_from_local_elevator */
